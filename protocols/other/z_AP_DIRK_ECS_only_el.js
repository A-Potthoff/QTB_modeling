/**
 * Macro for data evaluation of DIRK_ECS_el
...
// v_arrays[0][0]: the time distance between the measurement pulses in µs (constant - I would have used save_trace_time_scale but it does not work)
// v_arrays[0][1]: the number of pulses in the dark period
// v_arrays[1]: all the PARs tested (sequentially after another)
...
output.relaxation_fit_data_and_fit = unwrap_if_single(DIRK_ECS_out_relaxation_fit_data_and_fit); //only well-depicted in EDITOR and when only one PAR tested
// output.residuals_of_exponential_fit= unwrap_if_single(out_residuals_of_exponential_fit);
output.dark_relaxation_window =        unwrap_if_single(DIRK_ECS_out_dark_relaxation_window);
output.fitted_curve_exp_decay=       unwrap_if_single(DIRK_ECS_out_fitted_curve_exp_decay);
...
output.ECS_tau_in_ms =      unwrap_if_single(DIRK_ECS_out_ECS_tau);
output.v_Hplus_in_mAU_per_ms = unwrap_if_single(DIRK_ECS_out_v_Hplus);
output.DIRK_time_between_pulses_in_ms = DIRK_ECS_time_beween_pulses;
