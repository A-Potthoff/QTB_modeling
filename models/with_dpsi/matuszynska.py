...
def calculate_pHstroma(x): # not in use
...
def calculate_pH(x): # in use
    return -np.log10(x * 2.5e-4) 
# Andre does not understand, why 2.5e-4? it cannot be convf for concentrations from mmol/molChl into mol/L(lumen), so what is it?
# it cannot be that convf because it needs to be higher than confv. (see also parameter list below)
...
def vPTOX(Pred, time, kPTOX, ox, O2ext, kNDH, Ton, Toff):
    ...
    """
    O2 = oxygen(time, ox, O2ext, kNDH, Ton, Toff)[0]
    return Pred * kPTOX * O2
...
variables = [
    ...
    "dpsi", #delta psi across the thylakoid membrane, measured in mV
]

# I dont trust https://tinyurl.com/3sm82b3f: this is the true link: https://kirschner.med.harvard.edu/files/bionumbers/A%20semi%20quantitative%20analysis%20of%20the%20photosynthetic%20system%20in%20an%20'average'%20C3%20plant%20leaf.pdf
p = {
    "convf": 3.2e-2,  # converts ATP and NADPH from mmol/L in mmol/molChl. units: molChl/LChloroplast # fits observations very well. we would expect around 30 mol_Chl / L_chloroplast (see https://bionumbers.hms.harvard.edu/bionumber.aspx?s=n&v=3&id=100343)
    "convflumen": 0.5, # converts concentrations from mmol/molChl to mmol/L in thylakoid.       conversion factor has units molChl/Llumen
     # calculated with values taken from https://tinyurl.com/3sm82b3f
    ...
    "APtot": 2.55,  # [mmol/L] Bionumbers ~2.55mM (=81mmol/molChl)
    ...
    "VolumeLumen": 2.1e-15, # Liters per Chloroplast according to eg. https://doi.org/10.1016/j.febslet.2013.05.046 and https://tinyurl.com/3sm82b3f, also roughly consistent with Matuszynska 2019.
    "Cthylakoid": 1.2e-12, # capacitance of the thylakoid membrane;
    # "true" value: 4e-14 according to [Muttiah and Savenkov (2021), 10.1101/2021.06.30.450215],
    # "true" value: F/cm2 * cm2 = {[0.6,1]}e-6 * 5.4e-6 = 4e-12, taken from Cruz2001(10.1021/bi0018741) and thylakoid surface area from https://tinyurl.com/3sm82b3f
    # the used value was kept in this range and chosen such that dpsi stays within reasonable ranges.
    # rate constants
    ## b6f
    "b6f_content": 1,
    "max_b6f": 500,
    ...
    "kPCox": 2.5e3,  # a rough estimate: half life of PC->P700 should be ~0.2ms
    ...
    "E0_NADP": -0.113,
    # physical constants
    "F": 96.485,  # Faraday constant !!! in C/ mmol !!! 
    "R": 8.314e-3,  # universal gas constant !!! in # J/(mmol·K) !!!
    ...
    "Kast2": 0.02,
    "Kast3": 0.02,
    "k": 8e8,  # large k used for rapid equilibration approximation
    # CBB speedup factor
    "Km_fcbb": 150.0,
    "Vmax_fcbb": 6.0,
}
...
def conversion_protons2psi(convflumen_in_mmolChl_per_L_lumen, Volume_of_Lumen, Faraday_in_Coloumb_per_mmol, Capacitance_thylakoid_in_Coloumb_per_V):
    """
    interesting note: initially result was divided by 1e3 to convert fom V -> mV and readout in mV.
    but then fluxes were around 200, and simulation_to_steady_state() did not work anymore because system became to unstable.
    then, dpsi still changed too much after 2e6 seconds. 
    when measured in Volts, problem was still present in some instances. therefore we measure in kV.
    """
    lumbed_coefficient = convflumen_in_mmolChl_per_L_lumen * Volume_of_Lumen * Faraday_in_Coloumb_per_mmol / Capacitance_thylakoid_in_Coloumb_per_V / 1000 # 1000V = 1kV
    return lumbed_coefficient
...
def get_matusznyska() -> Model:
    ...
    # print(f"KeqFAFd:{m.get_derived_parameter_value('Keq_FAFd')}")
    ...
    # print(f"KeqPCP700:{m.get_derived_parameter_value('Keq_PCP700')}")
    ...
    m.add_derived_parameter(
        parameter_name="conv_psi",
        function=conversion_protons2psi,
        parameters=["convflumen","VolumeLumen", "F", "Cthylakoid"],
    )
    ...
    m.add_algebraic_module( # suggestino todo from Andre: move k_b6f computation to the rate function such that it is not a "compound" (by alm).
        module_name="k_b6f",
        function=k_b6f,
        compounds=["pH"],
    ...
    m.add_reaction_from_args(
        ...
        stoichiometry={"B1": -1,
                       "B2": 1,
                       "H": 1 / m.get_parameter("bH"), #watersplitting occurs on lumenal side and protons will be buffered in the lumen by amino acids
                       "dpsi": 1 * m.get_derived_parameter_value("conv_psi") # convert mmol/molChl to mV/chloroplast
                       },
    ...
    m.add_reaction(
        rate_name="vNDH", # used as a secondary electron source (those electrons could come from the mitochondrium for example)
        function=vNDH,
        stoichiometry={"PQ": -1},
        modifiers=["time"],
    ...
    #     stoichiometry={"PC": -2,
    #                    "PQ": 1,
    #                    "H": 4 / m.get_parameter("bH"),
    #                    "dpsi": 4 * m.get_derived_parameter_value("conv_psi") # convert mmol/molChl to mV/chloroplast
    #                    },
    ...
    m.add_reaction(
        ...
        stoichiometry={
            ...
            "dpsi": -m.get_parameter("HPR") * m.get_derived_parameter_value("conv_psi"),  # convert mmol/molChl to mV/chloroplast
